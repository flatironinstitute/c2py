...
namespace c2py {
  ...
  PyObject *numpy_proxy::to_python() {
    ...
    PyArrayObject *arr = (PyArrayObject *)(result); //NOLINT
#ifdef PYTHON_NUMPY_VERSION_LT_17
    arr->base = base;
  ...
  numpy_proxy make_numpy_proxy(PyObject *obj) {
    ...
    if (not PyArray_Check(obj)) return {}; //NOLINT

    numpy_proxy result;

    // extract strides and lengths
    PyArrayObject *arr = (PyArrayObject *)(obj); //NOLINT
...
#endif
    return PyArray_FromAny(obj, PyArray_DescrFromType(element_type), rank, rank, flags, nullptr); // //NOLINT new ref
  }
