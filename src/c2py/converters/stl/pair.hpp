...
namespace c2py {
  ...
  template <typename T1, typename T2> struct py_converter<std::pair<T1, T2>> {
    ...
    template <typename P> static PyObject *c2py(P &&p) {
      ...
      pyref x2 = cxx2py(std::get<1>(std::forward<P>(p)));

      if (x1.is_null() or x2.is_null()) return nullptr;
      return PyTuple_Pack(2, (PyObject *)x1, (PyObject *)x2); //NOLINT
    }

    static bool is_convertible(PyObject *ob, bool raise_exception) {
      if (!PySequence_Check(ob)) goto _false; //NOLINT
      {
        pyref seq = PySequence_Fast(ob, "expected a sequence");
        if (!py_converter<T1>::is_convertible(PySequence_Fast_GET_ITEM((PyObject *)seq, 0), raise_exception)) goto _false; // NOLINT orrowed ref
        if (!py_converter<T2>::is_convertible(PySequence_Fast_GET_ITEM((PyObject *)seq, 1), raise_exception)) goto _false; // NOLINT borrowed ref
        return true;
      }
    _false:
