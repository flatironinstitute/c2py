...
namespace c2py {
  ...
  template <typename... Types> struct py_converter<std::tuple<Types...>> {
    ...
    template <typename T, auto... Is> static PyObject *c2py_impl(T &&t, std::index_sequence<Is...>) {
      auto objs        = std::array<pyref, sizeof...(Is)>{cxx2py(std::get<Is>(std::forward<T>(t)))...};
      bool one_is_null = std::any_of(std::begin(objs), std::end(objs), [](PyObject *a) { return a == nullptr; });
      if (one_is_null) return nullptr;
