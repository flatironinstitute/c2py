...
namespace c2py {
  ...
  template <typename V> numpy_proxy make_numpy_proxy_from_vector(V &&v) {
    ...
    auto *vec_heap        = new std::vector<value_type>{std::forward<V>(v)}; // NOLINT
  ...
  template <typename T> std::vector<T> make_vector_from_numpy_proxy(numpy_proxy const &p) {
    C2PY_EXPECTS(p.extents.size() == 1);
    C2PY_EXPECTS(p.strides[0] % sizeof(T) == 0);
  ...
  template <> struct py_converter<std::vector<std::byte>> {
    ...
    static std::vector<std::byte> py2c(PyObject *ob) {
      ...
      auto *buffer = reinterpret_cast<std::byte *>(PyBytes_AsString(ob)); // NOLINT
      return {buffer, buffer + size};                                     // NOLINT
    }
  };
  ...
  template <typename T> struct py_converter<std::vector<T>> {
    ...
    static bool is_convertible(PyObject *ob, bool raise_exception) {
      ...
      if (PyArray_Check(ob)) {
        PyArrayObject *arr = (PyArrayObject *)(ob); // NOLINT
      ...
      long len  = PySequence_Size(ob);
      for (long i = 0; i < len; i++) {
        if (!py_converter<T>::is_convertible(PySequence_Fast_GET_ITEM((PyObject *)seq, i),
                                             raise_exception)) { // borrowed ref
          return false;
        }
      }
      return true;
    ...
    static std::vector<T> py2c(PyObject *ob) {
      // Special case: 1-d ndarray of builtin type
      if (PyArray_Check(ob)) {
        auto *arr = (PyArrayObject *)(ob); // NOLINT
#ifdef PYTHON_NUMPY_VERSION_LT_17
        int rank = arr->nd;
#else
        int rank = PyArray_NDIM(arr);
      ...
      C2PY_ASSERT(PySequence_Check(ob));
      std::vector<T> res;
      pyref seq = PySequence_Fast(ob, "expected a sequence");
      long len  = PySequence_Size(ob);
      for (long i = 0; i < len; i++) res.push_back(py_converter<std::decay_t<T>>::py2c(PySequence_Fast_GET_ITEM((PyObject *)seq, i))); //borrowed ref
      return res;
    }
  };
