  ...
  template <typename T, size_t R> struct py_converter<std::array<T, R>> {
    ...
    static bool is_convertible(PyObject *ob, bool raise_exception) {
      if (!PySequence_Check(ob)) goto _false; // NOLINT
      {
        ...
        Py_ssize_t len = PySequence_Size(ob);
        if (len != R) {
          if (raise_exception) {
        ...
        for (int i = 0; i < len; i++)
          if (!py_converter<T>::is_convertible(PySequence_Fast_GET_ITEM((PyObject *)seq, i), raise_exception)) goto _false; // NOLINT borrowed ref
